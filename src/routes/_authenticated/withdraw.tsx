...
function WithdrawPage() {
  ...
  const refresh = async () => {
    const [p, w, s, inv] = await Promise.all([
      ...
      supabase.from("investments").select("plan_amount").eq("status", "active"),
    ]);
    ...
    if (inv.data) setActiveMining(inv.data.reduce((sum, i) => sum + Number(i.plan_amount), 0));
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Stat label="Available balance" value={`KSh ${fmt(Math.floor(balance))}`} accent />
      ...
      <form onSubmit={submit} className="space-y-5 rounded-2xl border border-border/60 bg-card p-6">
        ...
        <button disabled={loading || balance < minW || closed} className="rounded-md bg-[image:var(--gradient-gold)] px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] disabled:opacity-60">
