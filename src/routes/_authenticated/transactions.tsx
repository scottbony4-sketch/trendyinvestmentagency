    ...
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-2xl border border-border/60">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">No transactions</td></tr>}
            {filtered.map(r => (
              <tr key={r.id} className="border-t border-border/40">
                ...
                <td className={`px-4 py-3 font-semibold ${r.type === "withdrawal" ? "text-red-400" : "text-emerald-400"}`}>{r.type === "withdrawal" ? "−" : "+"}KES {fmt(r.amount)}</td>
