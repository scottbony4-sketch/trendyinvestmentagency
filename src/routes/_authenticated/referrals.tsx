...
function ReferralsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!referralCode && p.data?.full_name) {
        const { data: generatedCode } = await supabase.rpc("gen_referral_code_from_name", { _full_name: p.data.full_name });
        if (generatedCode) {
  ...
  const link = normalizedCode ? `${getSiteUrl() || ""}/signup?ref=${encodeURIComponent(normalizedCode)}` : "";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Stat icon={Coins} label="Total earned" value={`KES ${fmt(earned)}`} />
      ...
      <section>
        ...
        <div className="mt-3 overflow-x-auto rounded-2xl border border-border/60">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={3} className="px-4 py-8 text-center text-muted-foreground">No referral earnings yet</td></tr>}
              {rows.map(r => (
                <tr key={r.id} className="border-t border-border/40">
                  ...
                  <td className="px-4 py-3 font-semibold text-emerald-400">+KES {fmt(r.amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
